...
st.sidebar.info("💡 **設定のポイント:**\nデフォルトはカタログ値のアルブミンSC=0.6と設定していますが、実際の治療(in vivo)では、タンパク付着(ファウリング)によりSCはカタログ値SC=0.6より低下：予測値よりアルブミンを喪失しない可能性があります。病因物質のSCのデフォルトは、エバキュアーEC-4AのIgGに対するカタログ値SC=0.4としています")
